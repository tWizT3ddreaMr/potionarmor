...
public class main extends org.bukkit.plugin.java.JavaPlugin {
    ...
    public void onEnable() {
        ...
        for (PotionEffectType pe : Registry.EFFECT) {
            Effect.add(pe.getKey().toString().replace("minecraft:","")); //default "minecraft:" namespace is assumed by the Effect registry
        }
    ...
    public void config() {
        if (configHandler.mconfig.contains("Version")
                && configHandler.mconfig.getInt("Version") == 1) return;
        ...
        for (String eff : Effect) {
            String eff2 = eff; //TODO: why are there 2 eff strings? and which should be used where?
            if (eff.equalsIgnoreCase("HEAL")) eff2 = "INSTANT_HEALTH";
            if (eff.equalsIgnoreCase("SLOW")) eff2 = "SLOWNESS";
            if (configHandler.mconfig.contains(eff)) continue;
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (command.getName().equalsIgnoreCase("reload")){
        	if(sender instanceof Player){
        		Player p = (Player) sender;
        		if (!p.hasPermission("Potionarmor.reload")){
            	sender.sendMessage(ChatColor.RED + "No permission.");
            	return true;
        		}
        	}
